...
const e = {
  ...
  usersalt: "<:usersalt:1539125796046639154>",
...
client.on('messageCreate', async (message) => {
    ...
    if (userMap.has(message.author.id)) {
      ...
      if (difference > TIME) {
        ...
        if (parseInt(msgCount) === LIMIT) {
          const punicao = (await db.get(`antispam_action_${message.guild.id}`)) || 'mute';

          if (punicao === 'mute') {
            await message.member.timeout(10 * 60 * 1000, 'Anti-Spam Automático').catch(() => {});
            message.channel.send(`${e.warn} ${message.author} foi mutado por 10 minutos devido ao spam!`);
          } else if (punicao === 'kick') {
            await message.member.kick('Anti-Spam Automático').catch(() => {});
            message.channel.send(`${e.proibido} ${message.author.tag} foi expulso do servidor por spam!`);
          } else if (punicao === 'ban') {
            await message.member.ban({ reason: 'Anti-Spam Automático' }).catch(() => {});
            message.channel.send(`${e.banido} ${message.author.tag} foi banido do servidor por spam!`);
          }
  ...
  // Comando fox.rank (Rank Auto-Atualizável)
...
// Render e Reset do Rank de Call
async function atualizarRankEmbed(message, guild) {
  ...
  if (Date.now() >= proxReset) {
    for (const key in allData) {
      if (key.startsWith(prefix) || key.startsWith(`voice_join_${guild.id}_`)) {
        await db.delete(key);
      }
    }
    proxReset = Date.now() + (diasReset * 24 * 60 * 60 * 1000);
    ...
    for (const key in allData) {
      if (key.startsWith(prefix)) delete allData[key];
    }
  }
  ...
  for (const key in allData) {
    if (key.startsWith(prefix)) {
      ...
      let tempoTotal = allData[key] || 0;

      const joinTime = await db.get(`voice_join_${guild.id}_${userId}`);
      if (joinTime) tempoTotal += (Date.now() - joinTime);
  ...
  if (top10.length === 0) {
    ...
    top10.forEach((item, index) => {
      const pos = index + 1;
      const posStr = pos === 1 ? '1º' : pos === 2 ? '2º' : pos === 3 ? '3º' : pos === 4 ? '4º' : pos === 5 ? '5º' : `${pos}º`;
      descricao += `${posStr} <@${item.userId}> — **${formatarTempoExtenso(item.tempo)}**\n\n`;
    });
  }
...
// Evento de Interações (/config, botões, modais)
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    if (valor === 'op_antispam') {
      const antispamSelect = new StringSelectMenuBuilder()
        .setCustomId('select_antispam_action')
        .setPlaceholder('Escolha a punição para Spam')
        .addOptions([
          { label: 'Ativar / Mute (10 min)', value: 'antispam_mute', description: 'Silencia o membro que fizer spam' },
          { label: 'Ativar / Expulsar (Kick)', value: 'antispam_kick', description: 'Expulsa o membro do servidor' },
          { label: 'Ativar / Banir (Ban)', value: 'antispam_ban', description: 'Bane o membro permanentemente' },
          { label: 'Desativar Anti-Spam', value: 'antispam_off', description: 'Desliga a proteção contra spam' }
        ]);

      return interaction.update({ content: `${e.protecao} **Configuração de Anti-Spam**\nEscolha o que acontecerá com quem floodar no chat:`, components: [new ActionRowBuilder().addComponents(antispamSelect)] });
    }

    if (valor.startsWith('antispam_')) {
      if (valor === 'antispam_off') {
        await db.set(`antispam_status_${interaction.guildId}`, false);
        return interaction.reply({ content: `${e.desativado} **Anti-Spam Desativado!**`, ephemeral: true });
      }

      const acao = valor.replace('antispam_', '');
      await db.set(`antispam_status_${interaction.guildId}`, true);
      await db.set(`antispam_action_${interaction.guildId}`, acao);

      const acaoNome = acao === 'mute' ? 'Mudar pra Mute (10min)' : acao === 'kick' ? 'Kickar (Expulsar)' : 'Banir';
      return interaction.reply({ content: `${e.ativado} **Anti-Spam Ativado!** Punição configurada: **${acaoNome}**.`, ephemeral: true });
    }
    ...
    if (valor === 'op_welcome') {
      ...
      await interaction.update({ content: `${e.perfil} **Canais de Boas-Vindas**`, components: [new ActionRowBuilder().addComponents(cSelect)] });
    }
    if (valor === 'op_logs_msg') {
      ...
      await interaction.update({ content: `${e.alerta} **Logs de Mensagem/Fotos**`, components: [new ActionRowBuilder().addComponents(cSelect)] });
    }
    if (valor === 'op_logs_call') {
      ...
      await interaction.update({ content: `${e.horario} **Logs de Call**`, components: [new ActionRowBuilder().addComponents(cSelect)] });
    }
  }
  ...
  if (interaction.isChannelSelectMenu()) {
    ...
    await interaction.reply({ content: `${e.positivo} Configuração salva!`, ephemeral: true });
  }
});
...
async function enviarPainelPrincipal(interaction) {
  const menu = new StringSelectMenuBuilder()
    ...
    .addOptions([
      { label: 'Sistema Anti-Spam (Kick/Ban/Mute)', value: 'op_antispam', emoji: '1534611991929290877' },
  ...
  const embed = new EmbedBuilder().setTitle(`${e.config} Painel de Configurações`).setColor("#2b2d31").setDescription('Selecione uma opção no menu abaixo para configurar.');
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (!oldState.channelId && newState.channelId) {
    ...
    if (joinTime) {
      ...
      const antigo = (await db.get(`voice_time_${guild.id}_${userId}`)) || 0;
