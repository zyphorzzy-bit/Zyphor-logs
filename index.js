...
// IDs autorizados a configurar e usar o comando .f
...
client.once('ready', async () => {
  ...
  // Restaura a transmissão salva no banco de dados ao ligar
...
// Comando Prefixo .f (Exemplo: .f Jogando algo)
client.on('messageCreate', async (message) => {
  ...
  if (message.content.startsWith('.f')) {
    ...
    // Define a presença como Transmitindo (Streaming)
    ...
    return message.reply(`${e.aceito} **Stream atualizada com sucesso para:** \`${novoTexto}\``);
  }
});

// Interações (Comandos de Barra e Menus)
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand() || interaction.isStringSelectMenu() || interaction.isChannelSelectMenu() || interaction.isModalSubmit()) {
    if (!AUTORIZADOS.includes(interaction.user.id)) {
      return interaction.reply({
        content: `${e.proibido} **Acesso Negado!** Apenas os administradores autorizados podem usar o painel.`,
        ephemeral: true
      });
    }
  }
  ...
  // Lógica do Modal de Edição de Stream no Painel
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    if (valor === 'menu_home') {
      ...
      const input = new TextInputBuilder()
        ...
        .setPlaceholder('Ex: Live no Youtube!')
        .setRequired(true);
      ...
      await interaction.update({
        content: `${e.user} **Módulo de Boas-Vindas**\n${e.seta} Selecione abaixo os canais onde as mensagens serão enviadas:`,
        components: [
      ...
      await interaction.update({
        content: `${e.horario} **Módulo de Logs de Call**\n${e.seta} Selecione o canal de logs de chamada de voz:`,
        components: [
          new ActionRowBuilder().addComponents(canalSelect),
          new ActionRowBuilder().addComponents(voltarBtn)
        ]
      ...
      await interaction.update({
        content: `${e.warn} **Módulo de Logs de Punições**\n${e.seta} Selecione o canal de logs de punições:`,
        components: [
          new ActionRowBuilder().addComponents(canalSelect),
          new ActionRowBuilder().addComponents(voltarBtn)
        ]
...
async function enviarPainelPrincipal(interaction, isUpdate = false) {
  ...
  const menu = new StringSelectMenuBuilder()
    .setCustomId('menu_principal_config')
    .setPlaceholder('Escolha um sistema para gerenciar...')
    .addOptions([
      { label: 'Alterar Stream', description: 'Mudar a mensagem de transmissão do bot', value: 'op_stream', emoji: '1539124690709385330' },
  ...
  const textoPainel = `${e.config} **PAINEL DE CONFIGURAÇÃO GERAL**\n\n` +
                      ...
                      `${e.seta} *Dica: Você também pode usar \`.f <texto>\` no chat para alterar a stream.*`;
