...
// ID do proprietário vinculado à licença no banco local
const CLIENTE_DONO_ID = "1533306874513068093"; 
...
// -------------------------------------------------------------
// FUNÇÃO DE VERIFICAÇÃO DE LICENÇA (JSONING LOCAL)
// -------------------------------------------------------------
async function checarTravaLicenca() {
  try {
    const licenca = await db.get(`licenca_${CLIENTE_DONO_ID}`);

    if (!licenca) {
      return { bloqueado: true, motivo: "Licença não registrada no banco local." };
    }

    if (licenca.desligado) {
      return { bloqueado: true, motivo: "Licença desativada manualmente." };
    }

    if (!licenca.permanente && Date.now() >= Number(licenca.expiracao)) {
      licenca.desligado = true;
      await db.set(`licenca_${CLIENTE_DONO_ID}`, licenca);
      return { bloqueado: true, motivo: "Prazo de licença expirado." };
    }

    return { bloqueado: false };
  } catch (err) {
    console.error("Erro na verificação do Jsoning:", err);
    return { bloqueado: true, motivo: "Erro de leitura no banco local." };
  }
}
...
client.once('ready', async () => {
  ...
  // Garante que a licença inicial exista no arquivo local para não bloquear o bot
  const licencaExistente = await db.get(`licenca_${CLIENTE_DONO_ID}`);
  if (!licencaExistente) {
    await db.set(`licenca_${CLIENTE_DONO_ID}`, {
      dono_id: CLIENTE_DONO_ID,
      desligado: false,
      permanente: true,
      expiracao: 0
    });
    console.log("Licença padrão criada com sucesso no arquivo database.json!");
  }
...
// EVENTO DE MENSAGEM COM VALIDAÇÃO DE TRAVA
client.on('messageCreate', async (message) => {
  if (message.author.bot || !message.guild) return;

  const isOwner = OWNERS_IDS.includes(message.author.id);

  // Execução da validação no Banco Local
  const checagem = await checarTravaLicenca();

  if (checagem.bloqueado && !isOwner) {
    const embedExpirado = new EmbedBuilder()
      .setTitle('<:warn:1539125781320433724> Sistema Bloqueado')
      .setDescription(`Esta aplicação encontra-se **desativada ou com a licença expirada**.\n\nEntre em contato com a equipe de suporte para renovar o acesso.`)
      .setColor('#ff3333')
      .setFooter({ text: "Sistema de Gerenciamento • Trava de Segurança" });
    ...
    return message.reply({ 
      ...
      components: [new ActionRowBuilder().addComponents(btnSuporte)] 
    });
  }
