...
// ID do proprietário vinculado ao registro da licença no banco
...
// -------------------------------------------------------------
// FUNÇÃO DE VERIFICAÇÃO DE LICENÇA (SUPABASE)
// -------------------------------------------------------------
async function checarTravaLicenca() {
  try {
    const { data: botDados, error } = await supabase
      .from('bots')
      .select('*')
      .eq('dono_id', CLIENTE_DONO_ID)
      .maybeSingle();

    if (error || !botDados) {
      return { bloqueado: true, motivo: "Licença não registrada no banco de dados." };
    }

    if (botDados.desligado) {
      return { bloqueado: true, motivo: "Licença desativada manualmente." };
    }

    if (!botDados.permanente && Date.now() >= Number(botDados.expiracao)) {
    ...
    console.error("Erro na verificação do Supabase:", err);
    return { bloqueado: true, motivo: "Erro de conexão com o banco de dados." };
  }
}
...
client.on('messageCreate', async (message) => {
  ...
  // Execução da validação no Supabase
