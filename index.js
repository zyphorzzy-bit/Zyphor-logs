...
// SISTEMA DE MENSAGENS E ANTI-SPAM (APENAS DELETA A MENSAGEM)
client.on('messageCreate', async (message) => {
  ...
  // Comando fox.rank (Auto-Atualizável)
...
// RANK DE CALL ATUALIZADO EM TEMPO REAL
async function atualizarRankEmbed(message, guild) {
  ...
  for (const key in allData) {
    if (key.startsWith(prefix)) {
      ...
      rankingMap.set(userId, allData[key] || 0);
    }
  }

  // Soma o tempo dos membros que estão em call AGORA no ranking
  const now = Date.now();
  for (const [userId, member] of guild.members.cache) {
    if (member.voice.channelId && !member.user.bot) {
      const joinTime = await db.get(`voice_join_${guild.id}_${userId}`);
      if (joinTime) {
        const tempoAtual = rankingMap.get(userId) || 0;
        rankingMap.set(userId, tempoAtual + (now - joinTime));
      }
    }
  }

  let ranking = [];
  for (const [userId, tempo] of rankingMap.entries()) {
