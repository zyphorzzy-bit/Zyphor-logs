...
async function atualizarRankEmbed(message, guild) {
  ...
  for (const key in allData) {
    if (key.startsWith(prefix)) {
      ...
      rankingMap.set(userId, Number(allData[key]) || 0);
    }
  }
  ...
  if (top10.length === 0) {
    ...
    top10.forEach((item, index) => {
      const pos = index + 1;
      descricao += `${pos}º <@${item.userId}> — **${formatarTempoExtenso(item.tempo)}**\n\n`;
    });
  }
...
// PAINEL /CONFIG COM BOTÃO VOLTAR
